...
st.set_page_config(page_title="Simple Recommendation App", page_icon="💡", layout="wide")
...
def prepare_recommendation_candidates():
    ...
    selected_customers = np.random.choice(curated_sales_events['CUSTOMER_ID'].unique(), size=10000, replace=False)
    ...
    product_features = vectorizer.fit_transform(selected_data['PRODUCT_TEXT'])
...
def compute_recommendations(customer_id, selected_data, cosine_sim, top_n=5):
    ...
    for main_category in customer_main_category:
        ...
        category_data = selected_data[selected_data['MAIN_CATEGORY'] == main_category]
...
def simple_recommendation_app(selected_customers, selected_data, cosine_sim):
    ...
    customer_id = st.selectbox("Select a Customer ID:", selected_customers[:50])
